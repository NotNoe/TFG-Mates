...
def calor1D(intervalo, f, alpha, beta, t_obj, nt, nx):
    ...
    Returns an np.matrix with the values in the grid, as well as the min and max values (just for graphing)
    If lambda is greater than 0.5, a message will be shown but  
    """
    M = -np.inf #Valor maximo, inicialmente infinito
    m = np.inf #Valor minimo, inicialmente menos infinito
    ...
    if(lam > 1/2):
        print("Warning: dt/dx² must be less or equal than 0.5, and it's ", lam, ", the method might not converge.")
        print("dt = ", dt)
        print("dx = ",dx)
